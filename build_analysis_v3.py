...
# ── 叙事阶段解析（从 story_profile.json 加载）──
_STORY_STAGES = []
def _load_story_stages():
    global _STORY_STAGES
    if _STORY_STAGES:
        return _STORY_STAGES
    import json as _json
    sp_path = os.path.join(_project["work_dir"], "story_profile.json")
    if os.path.exists(sp_path):
        with open(sp_path, 'r', encoding='utf-8') as f:
            data = _json.load(f)
        nodes = data.get("story_nodes", data.get("情节节点", []))
        for node in nodes:
            ep_range = node.get("集数范围", "")
            nums = __import__('re').findall(r'\d+', str(ep_range))
            if len(nums) >= 2:
                _STORY_STAGES.append({
                    "name": node.get("阶段", ""),
                    "ep_lo": int(nums[0]),
                    "ep_hi": int(nums[1]),
                    "desc": node.get("描述", "")[:60],
...
def _resolve_story_stage(ep):
    stages = _load_story_stages()
    for s in stages:
        if s["ep_lo"] <= int(ep or 0) <= s["ep_hi"]:
            return s["name"]
    return "未知"
...
def enrich(data, parsed):
    ...
    # ── 叙事阶段标签（基于 story_profile.json 的集数范围匹配）──
    data["story_stage"] = _resolve_story_stage(data.get("ep", 0))
