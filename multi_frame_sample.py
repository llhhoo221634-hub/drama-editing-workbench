...
返回格式:
{
  ...
  "aesthetic": {
    "composition_type": "对称/三分法/引导线/中心对称/无",
    "color_palette": "暖/冷/高饱和/低饱和/自然",
    "light_beauty": "电影级/良好/普通/差"
  },
  "scene": "场景",
  "light": "光线",
...
[美学评估规则 - aesthetic]
composition_type 构图类型判断标准:
  - 对称: 画面左右/上下元素近乎镜像对称，如两人面对面、建筑中轴线
  - 三分法: 主体位于画面1/3或2/3处，有明显的三分线分割
  - 引导线: 有明确线条(道路/栏杆/目光)将视线引向主体
  - 中心对称: 主体居中且画面元素围绕中心呈放射/环绕状
  - 无: 无明显构图特征

color_palette 色调判断标准:
  - 暖: 主色调偏红/橙/黄，如黄昏、烛光、暖色滤镜
  - 冷: 主色调偏蓝/青/紫，如夜景、阴天、冷色滤镜
  - 高饱和: 色彩浓烈鲜艳，对比度高
  - 低饱和: 色彩淡雅/近乎黑白，灰度明显
  - 自然: 无明显色彩偏向，接近现实自然色

light_beauty 光影质量级别定义:
  - 电影级: 有明显的主次光源、轮廓光、柔光或戏剧性光影对比(如伦勃朗光)
  - 良好: 光线均匀、主体清晰、无明显过曝或死黑
  - 普通: 基本可见但平淡，无层次感
  - 差: 严重过曝/欠曝、大面积死黑、或光线混乱刺眼"""
...
def call_vision_api(image_paths):
    ...
    payload = {
        ...
        "stream": False,
        "max_tokens": 1200 if len(image_paths) > 1 else 600,
    }
...
def flatten_v2_for_output(v2_data, ep_num, timestamp, frame_idx, prev_frame_id):
    ...
    # 审美标签（VL 直接输出）
    if 'aesthetic' in v2_data and v2_data['aesthetic']:
        flat['aesthetic'] = v2_data['aesthetic']
